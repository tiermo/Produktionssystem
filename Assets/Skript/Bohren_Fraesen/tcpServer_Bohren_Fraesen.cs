...
public class tcpServer_Bohren_Fraesen : MonoBehaviour
{
    private int port = 6901;
    ...
    void Start()
    {
        port = transform.parent.gameObject.GetComponent<ConstructorClient_Bohren_Fraesen>().getModulPortNr();
        if (port == 0)
        {
            port = 6901;
        }
    ...
    {  //process requests depending on string message received
        ...
        if (data.Contains("down"))
        {
            ...
            GetComponent<BohrenFraesenSkript>().moveDown(depth);
        }
        if (data.Contains("up"))
        {
            int spaceposition = data.IndexOf(' ');
            int depth = int.Parse(data.Substring(spaceposition + 1));
            GetComponent<BohrenFraesenSkript>().moveUp(depth);
        }
        if (data.Contains("left"))
        {
            int spaceposition = data.IndexOf(' ');
            float distance = float.Parse(data.Substring(spaceposition + 1));
            GetComponent<BohrenFraesenSkript>().moveLeft(distance);
        }
        if (data.Contains("right"))
        {
            int spaceposition = data.IndexOf(' ');
            float distance = float.Parse(data.Substring(spaceposition + 1));
            GetComponent<BohrenFraesenSkript>().moveRight(distance);
        }
        if (data.Contains("speed"))
        {
            int spaceposition = data.IndexOf(' ');
            string speed = data.Substring(spaceposition + 1);
            GetComponent<BohrenFraesenSkript>().SpeedSelect(speed);
        }
        if (string.Compare(data, "middle") == 0)
        {
            GetComponent<BohrenFraesenSkript>().moveMiddle();
        }
        if (string.Compare(data, "trigger") == 0)
        {
            GetComponent<BohrenFraesenSkript>().callTrigger();
        }
        if (string.Compare(data, "scale") == 0)
        {
            GetComponent<BohrenFraesenSkript>().callScale();
        }
        if (string.Compare(data, "on") == 0)
        {
            GetComponent<BohrenFraesenSkript>().turnOn();
        }
        if (string.Compare(data, "off") == 0)
        {
            GetComponent<BohrenFraesenSkript>().turnOff();
        }
        if (string.Compare(data, "stop") == 0)
        {
            GetComponent<BohrenFraesenSkript>().stopMovement();
        }
        if (string.Compare(data, "limitL") == 0)
        {
            GetComponent<BohrenFraesenSkript>().callLeftPosSensor();
        }
        if (string.Compare(data, "limitR") == 0)
        {
            GetComponent<BohrenFraesenSkript>().callRightPosSensor();
        }
        if (string.Compare(data, "limitU") == 0)
        {
            GetComponent<BohrenFraesenSkript>().callLimitSensorUp();
        }
        if (string.Compare(data, "limitD") == 0)
        {
            GetComponent<BohrenFraesenSkript>().callLimitSensorDown();
        }
        if (string.Compare(data, "limitltr") == 0)
        {
            GetComponent<BohrenFraesenSkript>().callMiddleLtRSensor();
        }
        if (string.Compare(data, "limitrtl") == 0)
        {
            GetComponent<BohrenFraesenSkript>().callMiddleRtLSensor();
        }
        if (string.Compare(data, "st") == 0)
        {
            StreamWriter writer = new StreamWriter(client.tcp.GetStream(), Encoding.ASCII);
            data = GetComponent<BohrenFraesenSkript>().getMachineStatus().ToString();
