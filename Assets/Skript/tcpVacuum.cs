...
//tcpVacuum controls tcp communication for vacuum machines
public class tcpVacuum : MonoBehaviour {
	public int port = 6401;
	public string vacuumOn = "10301010";        // service number definitions
    public string vacuumOff = "10301011";
	...
	private void onIncoming (ServerClient client, string data) {  //process requests depending on string message received
		if(string.Compare(data, "on")==0) {
			GetComponent<vacuumMachineScript> ().turnMachineOn ();
			sendBackMessage (vacuumOn);

		}
		if(string.Compare(data, "off")==0) {
			GetComponent<vacuumMachineScript> ().turnMachineOff ();
			sendBackMessage (vacuumOff);
		}
		if(string.Compare(data, "st")==0) {
			StreamWriter writer = new StreamWriter (client.tcp.GetStream (), Encoding.ASCII);
			data = GetComponent<vacuumMachineScript> ().getMachineStatus().ToString();
