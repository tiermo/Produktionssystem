...
public class tcpSensorMid_StartConveyorBelt : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        port = g.GetComponent<ConstructorClient_StartConveyorBelt>().getSensorPortMidNr();
        if (port == 0)
        {
            port = 6330;
        }
    ...
    {   //process requests depending on string message received
        ...
        {
            GetComponent<sensorMid_StartConveyorBelt>().setMonitorFlag();
        }
    }
