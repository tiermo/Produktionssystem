...
public class tcpSensorStart_StartConveyorBelt : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        port = g.GetComponent<ConstructorClient_StartConveyorBelt>().getSensorPortStartNr();
        if (port == 0)
        {
            port = 6300;
        }
    ...
    {   //process requests depending on string message received
        ...
        {
            GetComponent<sensorStart_StartConveyorBelt>().setMonitorFlag();
        }

        if (string.Compare(data, "st_off") == 0)
        {
            GetComponent<sensorStart_StartConveyorBelt>().deactiveMonitorFlag();
        }
    }
