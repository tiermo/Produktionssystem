...
public class tcpServer_StartConveyorBelt : MonoBehaviour {
	...
	void Start()
    {
        port = GetComponent<ConstructorClient_StartConveyorBelt>().getConveyorPortNr();
	...
	public void sendBackMessage (string data) {                    // send service number as acknowledgement
		StreamWriter writer = new StreamWriter (client.tcp.GetStream (), Encoding.ASCII);
		writer.WriteLine(data);
		writer.Flush();
	}
