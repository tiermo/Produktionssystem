...
public class tcpSensorStart_ConveyorBelt : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        port = g.GetComponent<ConstrutorClient_ConveyorBelt>().getSensorPortStartNr();
        if (port == 0)
        {
            port = 6301;
        }
    ...
    {   //process requests depending on string message received
        ...
        {
            GetComponent<sensorStart_ConveyorBelt>().setMonitorFlag();
        }

        if (string.Compare(data, "st_off") == 0)
        {
            GetComponent<sensorStart_ConveyorBelt>().deactiveMonitorFlag();
        }
    }
