...
public class Start_OmniConveyorControl : MonoBehaviour {
	...
	void Update () {
		...
		if (monitorFlag && isObjectOnConveyor) {            // send acknowledgement id presence sensor is called
			GetComponent<tcpServer_StartOmni> ().onObjectDetection ();
			monitorFlag= false;
		}
	}
	...
	public void moveLeft (string speed) {                               // function to move an object to the left
        ...
        GetComponent<tcpServer_StartOmni>().sendBackMessage("finished");
    }
