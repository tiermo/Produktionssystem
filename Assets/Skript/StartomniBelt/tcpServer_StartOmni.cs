...
public class tcpServer_StartOmni : MonoBehaviour
{
    ...
    void Start()
    {
        port = GetComponent<ConstructorClient_StartOmni>().getOmniPortNr();
        if (port == 0)
        {
            port = 6200;
            Debug.Log("port " + port);
    ...
    {  //process requests depending on string message received
        ...
        {
            GetComponent<Start_OmniConveyorControl>().setMonitorFlag();
        }
        else
        {
            ...
            {
                GetComponent<Start_OmniConveyorControl>().moveLeft(speed);
            }
            if (string.Compare(direction, "right") == 0)
            {
                GetComponent<Start_OmniConveyorControl>().moveRight(speed);
            }
            if (string.Compare(direction, "up") == 0)
            {
                GetComponent<Start_OmniConveyorControl>().moveUp(speed);
            }
            if (string.Compare(direction, "down") == 0)
            {
                GetComponent<Start_OmniConveyorControl>().moveDown(speed);
            }
        }
    }
