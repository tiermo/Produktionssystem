...
public class ConstructorClient_StapelMagazin : MonoBehaviour
{
    ...
    private GameObject t;   //son object "Magazin"

    void Start()
    {
        t = transform.Find("Magazin").gameObject;
        data = GetComponent<Drag_StapelMagazin>().SendInfo();
        ConnectToServer();
        Send(data);
    }
    ...
    private void OnIncomingData(string data)
    {
        if (data.Contains("/"))
        {
            ...
            {
                t.GetComponent<tcpServer_StapelMagazin>().enabled = true;
            }
        }
        else
        {
